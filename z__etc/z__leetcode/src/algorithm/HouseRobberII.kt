...
class HouseRobberII {
    ...
    fun rob(nums: IntArray): Int {
        ...
        val tmp = nums[0]
        val something = tmp + doRobbing(nums.slice(2 until nums.size - 1).toIntArray())

        val tmp2 = doRobbing(nums.slice(1 until nums.size).toIntArray())

        return kotlin.math.max(something, tmp2)
    }
}